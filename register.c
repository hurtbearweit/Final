...
RETURN:无
************************************************************************/
void  Register_Lighten(int x1,int y1,int x2,int y2,int flag)
{
	clrmous(MouseX,MouseY);
	delay(10);
...
RETURN:判断结果
************************************************************************/
void Data(char* A,char* I,char* P)           //将新注册的用户信息写入文件
{   int  l;
    int  i;
	FILE* fp;
	USER* u;
...
RETURN:判断结果
************************************************************************/
int R_ID_Jundge(char *I)               //判断输入的身份证号是否已注册过
{
	...
	if(flag==1)
	{
	    ...
	    for(i=0;i<l;i++)                                         //遍历读取到的所有用户
	    {
		    ...
		    fseek(fp,i*sizeof(USER),SEEK_SET);
		    fread(u,sizeof(USER),1,fp);
		    if(strcmp(u->ID,I)==0)                          //如果当前身份证在文件中已经存在
		    {
			    setfillstyle(1,DARKGRAY);
		        bar(550,90,640,106);
		        puthz(550,90,"该身份证已注册",16,16,RED);
			    if(u != NULL)
			    {
				    free(u);
				    u = NULL;
