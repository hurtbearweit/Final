...
{   FILE *fp;
	...
	while(1)
	{
		...
		if(MouseX>490&&MouseX<590&&MouseY>425&&MouseY<460)//在我的预约上
		{  
...
{   
	...
	puthz(220,20,"鼠标按任意处返回至上一界面",16,16,RED);
	puthz(100,50,"特殊项目名称",24,24,RED);
	puthz(300,50,"特殊餐饮服务预约",24,24,RED);
	puthz(100,100,"项目价格",24,24,RED);
	puthz(300,100,"免费",24,24,RED);
...
void My_Special_Book(int *page,int *num)
{
	...
	while(1)
	{
		...
		if(j!=0)
		{
			for(m=0;m<j;m++)            
			{
				switch(mark[m])
				{
					...
					case 1: 
						graph(20,50*(m+1),620,50*(m+2));
						puthz(45,50*(m+1)+20,"特殊表演",24,24,BLUE);
					...
					case 2:
						graph(20,50*(m+1),620,50*(m+2));
						puthz(45,50*(m+1)+20,"会员导览",24,24,BLUE);							
						puthz(195,50*(m+1)+20,"梦想大道",24,24,BLUE);
					...
					case 3:
						...
						puthz(80,50*(m+1)+20,"体验",24,24,BLUE);
					...
					case 4:
						graph(20,50*(m+1),620,50*(m+2));
						puthz(45,50*(m+1)+20,"特色过山车",24,24,BLUE);
		...
		while(1)
		{
			...
			if(MouseX>260&&MouseX<320&&MouseY>380&&MouseY<420)//在返回键上
			{  
...
void Draw_My_Special_Book()
{
	...
	puthz(262,388,"返回",24,32,LIGHTCYAN);
}
